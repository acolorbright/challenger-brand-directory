# Challenger Brand Directory — v2 Framework
...
> **v2 note:** This version refines the Disruptor and Innovator archetype descriptions for sharper, more differentiated brand concepts. Use this file instead of `challenger.md` when adding new markets.
...
| **Disruptor** | Push + Redefine | Changes the rules by elevating a dimension that matters but isn't the main battlefield. Don't talk about what everyone else is talking about — if the market talks specs, talk style. Pick a real dimension, just not the dominant one, and make it the whole game. |
| **Realkeeper** | Counter + Iterate | Back to basics. Strips away bloat. The OG version of the category, done with care. |
| **Innovator** | Push + Iterate | Wins on the industry's own terms — but outperforms everyone. Identifies the dimension the market already competes on and pushes it further. If the market is about speed, be faster. If it's about creativity, be more creative. No rule changes — just clear dominance. |
...
### Misfit
Rejects the dominant dimensions entirely. Asks: what does this market ignore or dismiss? Build everything around that. The Misfit doesn't try to win the existing game — it questions why the game is being played at all.

### Disruptor
Picks a real but non-dominant dimension and makes it the entire brand. The key move: identify something the market acknowledges as valuable but doesn't compete on seriously — then go all in. The Disruptor doesn't improve the existing battlefield; it shifts the conversation to a different one.

- Good Disruptor logic: "Everyone competes on X. We compete on Y — which also matters, just nobody talks about it."
- Bad Disruptor logic: "We're just doing X differently" (that's an Innovator) or "We reject X entirely" (that's a Misfit).

### Realkeeper
Strips away complexity, noise, and trend-chasing. Asks: what was this category really about before it got overcomplicated? Competes on the core dimensions — but through reduction and honesty rather than performance.

### Innovator
Takes the dimension(s) the market already competes on and wins there. No repositioning, no counter-positioning — just superior execution on what the market already values. The Innovator doesn't change the conversation; it dominates it.

- Good Innovator logic: "The market competes on X. We're simply the best at X."
- Bad Innovator logic: "We do X but also add Y" (scope creep) or "We do X differently" (that's a Disruptor).
...
Follow the exact data structure in challenger-v2.md. Analyze the norms carefully — for software markets use software-specific norm dimensions (UX paradigm, pricing model, assumed user, feature logic). Then develop all four archetypes with creative, brand-ready names.

For the Disruptor: identify a dimension that is real and relevant in this market but not the primary battleground — then build the entire brand around that dimension. Do not improve the main dimension; shift the conversation.

For the Innovator: identify the dimension the market already competes on most intensely, and win there through clear superiority. No repositioning — just better execution of what the market already values.
...
- [ ] **Disruptor**: competes on a real but non-dominant dimension — not the market's main battlefield
- [ ] **Innovator**: wins on the market's existing dominant dimension — no rule changes
- [ ] Market added to correct category and sorted alphabetically
- [ ] No JS syntax errors (check with validation command below)
